...
tf.app.flags.DEFINE_string('checkpoint_dir','birds_valid299/model.ckpt',
                           """Path where to read model checkpoints.""")

tf.app.flags.DEFINE_string('image_folder', 'atnngan-clc-sn-evaluation-generated-ima/attn_cl_sn_output/valid/single',
...
def preprocess(img):
    ...
    #img = img.resize((299, 299, 3))# = scipy.misc.imresize(img, (299, 299, 3),
                              #interp='bilinear')
                ...
                for name in files1:
                    if name.rfind('jpg') != -1 or name.rfind('png') != -1:
                        ...
                        if os.path.isfile(filename):
                            ...
                            #img = img.load()
                            #img = np.array(imageio.imread(filename), dtype=float)
